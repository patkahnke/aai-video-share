...
Proof API module contains classes for consuming the Proof API and displaying the results:
 ...
 - NowPlayingBlock, TopTenViewsBlock, and TopTenVotesBlock display lists of videos that can be displayed on various pages
...
======================
 ...
 - It would be preferable if the videos that are played by clicking a link in one of the Video List blocks would play
    ...
    the video that is displayed, but it still requires a second click to actually play the video. The addition of "&autoplay=1"
    to the embed url is not giving the desired result.
